# Hi <img src="./images/Hi.gif" width="30px"> I'm Yogesh

**AI advisor, trainer and researcher.** I help organizations and individuals navigate AI, teach
data science and quantum technologies, write books in Marathi and English, and do independent
research on geometric deep learning.

## I am involved in:

### 🤖 AI Advisory Services
I run an Artificial Intelligence (AI) advisory service in the following modes:

* **Consultancy**: Helping organizations in the evolving AI landscape. 👉 More details (including commercials) [here](./domains/Notes_AIAdvisory.md).
* **Training**: Helping individuals stay current with AI, ML, NLP, etc.  👉 More details (including commercials) [here](./domains/Notes_AITraining.md). The educational content used in training, covering Python, Machine Learning, Deep Learning, etc., has been open-sourced in the [`TeachingDataScience`](https://github.com/yogeshhk/TeachingDataScience) repository.
* **Recruitment**: Helping organizations in hiring pre-screened candidates with AI skills. 👉 More details (including commercials) [here](./domains/Notes_AIRecruitment.md)

### 🔬 Research and Collaboration
I actively engage in personal research and collaborations with academic and industry partners on topics such as:
* ✅ **Geometric Deep Learning**
  Creating invariant representations for dimensionality reduction, see [MidcurveNN](https://github.com/yogeshhk/MidcurveNN).
  I am currently looking for **MS / PhD students** to take this further — the open problem is
  predicting geometry natively as graphs, where node positions can be learned but topology so far
  cannot. [Two-page topic proposal](https://github.com/yogeshhk/MidcurveNN/blob/master/publications/Midcurve_LaTeX/Main_TwoPager_MidcurveNN_GeometryResearch.pdf).

### 📚 Books and Course Material
* **Books** I write and self-publish. Sources and drafts live in [`Publications`](https://github.com/yogeshhk/Publications) — currently an Artificial Intelligence / Mental Models / Game Theory trilogy, plus **AI-Rupe**, a collection of my Marathi AI articles (available at [Amazon India](https://www.amazon.in/dp/B0FB37F7Z7), [Flipkart](https://www.flipkart.com/ai-rupe/p/itm8e9a8104bed35?pid=9798899613739&affid=editornoti), [Notion Press](https://direct.notionpress.com/in/read/ai-rupe/)).
* **Data Science / AIML** Open-sourced teaching material at [`TeachingDataScience`](https://github.com/yogeshhk/TeachingDataScience).
* **Quantum Technologies** Course material (LaTeX + LMS content) at [`TeachingQuantumTech`](https://github.com/yogeshhk/TeachingQuantumTech).
* **Indian Knowledge Systems** A completed study archive, including a full Patanjali Yoga Sutra seminar system (1 workshop + 6 seminars with cheat sheets), at [`BharatVidya`](https://github.com/yogeshhk/BharatVidya).

### 🙌 Social Contributions (Pro Bono)
I strongly believe in giving back to society and raising public awareness about AI through:
* 🧑‍🏫 **Pune AI Community** Social group dedicated to disseminating AI-related knowledge, both online and offline (Pune) [website](http://puneaicommunity.org/), [GitHub](https://github.com/puneaicommunity/puneaicommunity).
* 👯 **Mid-Career Transition Counseling (to Data Science)**. [My Story](https://www.youtube.com/watch?v=IQzWosVzkM4) | [TEDx Talk](https://www.youtube.com/watch?v=-VbWRs7BsPY) | [Path](https://www.youtube.com/watch?v=-_8i6IQz8l8)
* 💬 **Open Office Hours** Saturdays, 1:30–3:30 PM IST, for technical or career coaching.
* 📫 **LinkedIn** I regularly post about AI/ML and related topics. I’d love it if you [follow me](https://www.linkedin.com/in/yogeshkulkarni/) there. 😊
* ⚡ **Medium Blogs**  Read or follow my writings [here](https://yogeshharibhaukulkarni.medium.com/)
* 🎤 **Public Talks**  I speak at various forums and events. Check out my [YouTube playlist](https://www.youtube.com/playlist?list=PLaTX75s8-K31QUTU1h12Tzjuxl5nXjAG7) of talks.
* 🧑‍🏫 **Advisory** Internship for eng students, aspirants of 'Road to GDE (Google Developer Expert)', and member of BoS (Board of Studies)

Feel free to reach out via email at: `my firstnamelastname at yahoo dot com`

Thank you! 🙏



